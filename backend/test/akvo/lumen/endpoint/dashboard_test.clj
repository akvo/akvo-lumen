...
(deftest ^:functional dashboard
  ...
  (testing "Dashboard"
    (let [v-id               (-> (all-visualisations test-conn) first :id)
          ...
          {dashboard-id :id} (second (dashboard/create test-conn d-spec))]
      (is (not (nil? dashboard-id)))

      (testing "Get dashboard"
        (let [d (second (dashboard/fetch test-conn dashboard-id))]
          (is (not (nil? d)))
          (is (every? #(contains? d %)
      ...
      (testing "Update dashboard"
        (let [new-spec (-> d-spec
          ...
          (let [updated-d (second (dashboard/fetch test-conn dashboard-id))]
            (is (= (:title updated-d)

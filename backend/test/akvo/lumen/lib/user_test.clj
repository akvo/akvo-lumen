...
(def keycloak-config (let [c (tu/start-config)]
                       (merge (select-keys (:akvo.lumen.component.keycloak/authorization-service c) [:credentials :max-user-ids-cache])
                              {:public-client (:akvo.lumen.component.keycloak/public-client c)}
                              )))

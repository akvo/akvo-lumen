...
(defn apply-operation 
  ...
  (jdbc/with-db-transaction [conn tenant-conn]
    (let [{:keys [onError op args]} op-spec
          ...
          selected-column           (find-column (walk/keywordize-keys (:columns dsv)) (-> args :selectedColumn :columnName))
          ...
          new-columns (->> (columns-to-extract (:columns args) selected-column caddisfly-schema (:extractImage args))
                           (multiple-column/add-name-to-new-columns (:columns dsv)))]
      (if-let [errors (->> (map :title new-columns)
                           (filter #(engine/column-title-error? % (:columns dsv)))
                           not-empty)]
        {:success? false
        ...
        (let [_ (log/debug ::apply-operation (:table-name dsv) (:columnName selected-column) onError)
              ...
              update-db-columns (->> (db.transformation/select-rnum-and-column conn {:table-name (:table-name dsv) :column-name (:columnName selected-column)})
                                     ...
                                     doall)
              new-columns (into (:columns dsv) (vec new-columns))]

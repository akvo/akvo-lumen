...
(defn execute-transformation
  ...
  (let [dataset-versions (db.transformation/n-latest-dataset-version-by-dataset-id tenant-conn {:dataset-id dataset-id})]
...
(defn execute-undo [{:keys [tenant-conn] :as deps} dataset-id job-execution-id]
  (let [current-dataset-versions (db.transformation/n-latest-dataset-version-by-dataset-id tenant-conn {:dataset-id dataset-id})]
    ...
    (if-let [transformation (first transformations)]
      (let [transformation       (adapt-transformation transformation old-columns columns)
        ...
        (if avoid-tranformation?
          ...
          (let [op (try-apply-operation {:tenant-conn conn :caddisfly caddisfly} [{:table-name table-name :columns columns}] (assoc transformation :dataset-id dataset-id))]
            (when-not (:success? op)
              (throw

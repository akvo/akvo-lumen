...
(defn get-source-dataset [conn source]
  (let [source-dataset-id (get source "datasetId")]
    (if-let [source-dataset (first (filter #(= "main" (:namespace %)) (db.transformation/n-latest-dataset-version-by-dataset-id conn {:dataset-id source-dataset-id})))]
      source-dataset
...
(defn consistency-error? [tenant-conn dataset-id]
  (let [merged-sources (->> (db.transformation/n-latest-dataset-version-by-dataset-id tenant-conn {:dataset-id dataset-id})
                            (map :transformations)

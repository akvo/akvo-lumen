...
(defn encrypt
  ...
  (let [cipher (Cipher/getInstance "AES/ECB/PKCS5Padding")
        key-bytes (DigestUtils/sha256 ^String secret)
...
(defn decrypt
  ...
  [secret #^String cipher-text]
  {:pre [(string? secret) (string? cipher-text)
         (Base64/isBase64 (.getBytes ^String cipher-text))]
   :post [(string? %)]}
  (let [cipher (Cipher/getInstance "AES/ECB/PKCS5Padding")
        key-bytes (DigestUtils/sha256 ^String secret)
        key-spec (SecretKeySpec. key-bytes "AES")
        _ (.init cipher Cipher/DECRYPT_MODE key-spec)
        cipher-bytes (Base64/decodeBase64 ^String cipher-text)

     ...
     (try
       (with-open [importer (common/dataset-importer (get spec "source")
         ...
         (let [columns        (map  (fn [c] (update c :namespace (fn [ns] (or ns "main")))) (p/columns importer))
           ...
           (doseq [[record-groups parent-rnum] (map vector (take common/rows-limit (p/records importer)) (map inc (range)))]
       ...
       (catch Throwable e
         (failed-execution conn job-execution-id (.getMessage e) (into [(get @ns-table-names-atom "main")]
                                                                       (vals (dissoc @ns-table-names-atom "main" ))))
         (log/error e)

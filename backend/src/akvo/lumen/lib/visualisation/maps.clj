...
(defn create-raster [tenant-conn windshaft-url raster-id]
  (let [{:keys [raster_table metadata]} (db.raster/raster-by-id tenant-conn {:id raster-id})
        ...
        layer-meta (map-metadata/buildo tenant-conn raster_table {:layerType "raster"} nil nil)]
         ...
         (let [current-layer-type (:layerType current-layer)
           ...
           (if (= current-layer-type "raster")
             ...
             (let [ds-versions          (db.dataset/dataset-by-id tenant-conn {:id current-dataset-id})
               ...
               (map-metadata/buildo tenant-conn
                                    (or (seq table-names)
                                        (throw
...
(defn create
  ...
  (try
    ...
    (let [metadata-array (metadata-layers tenant-conn layers opts)
          ...
          map-config (map-config/builde tenant-conn layers metadata-array)
          headers* (headers tenant-conn)

...
(defn shape-aggregation-sql
  ...
  (let [aggregation-ds-versions (db.dataset/dataset-by-id tenant-conn {:id (:aggregationDataset current-layer)})
        point-columns (reduce into [] (map :columns aggregation-ds-versions))
        point-table-names (map :table-name aggregation-ds-versions)

        shape-ds-versions (db.dataset/dataset-by-id tenant-conn {:id (:datasetId current-layer)})

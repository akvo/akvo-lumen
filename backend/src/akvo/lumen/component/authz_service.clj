...
(defrecord AuthServiceClientImpl [http-connection-manager authz-service-url headers-fn]
  p/AuthServiceClient
  (check-permissions [this authz-credentials body]
...
(s/def ::http-service-client (partial instance? p/AuthServiceClient))

(defmethod ig/pre-init-spec ::http-service-client [_]
  (s/keys :req-un [::url ::api-headers-fn]))

...
(defn remove-group
  ...
  [kc id]
  (http/delete* (format "%s/groups/%s" (:api-root kc) id)
...
(defn get-groups
  ...
  [kc]
  (:body (http/get* (format "%s/groups" (:api-root kc))
                     {:headers (keycloak/request-headers kc)
                      :as :json})))
...
(defn get-clients
  ...
  [kc]
  (:body (http/get* (format "%s/clients" (:api-root kc))
                     {:headers (keycloak/request-headers kc)
                      :as :json})))
...
(defn update-client
  ...
  [kc client]
  (http/put* (format "%s/clients/%s" (:api-root kc) (:id client))
              {:headers (keycloak/request-headers kc)

...
-- :doc All persisted views by viz-is and dsv-id
   SELECT persisted_view.id, persisted_view.visualisation_id as "visualisation-id", data_group.columns, data_group.group_id as "group-id", data_group.group_name as "group-name", data_group.table_name as "table-name"
   FROM persisted_view

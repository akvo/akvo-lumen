...
v.id AS view_id, v.dataset_name, s.id AS source_id, i.id AS import_id, i.noble, i.status
FROM dataview v
...
SELECT DISTINCT ON (v.id)
v.id AS view_id, v.dataset_name, s.id AS source_id, i.id AS import_id, i.noble, t.id AS tranformation_id, t.fns, i.digest, i.status
FROM dataview v
LEFT JOIN datasources s ON v.datasource = s.id
LEFT JOIN (
     SELECT DISTINCT ON (i.datasource)
...
-- SELECT DISTINCT ON (v.id)
-- v.id AS view_id, v.dataset_name, s.id AS source_id, i.id AS import_id, i.noble
-- FROM dataview v
-- LEFT JOIN datasources s ON v.datasource = s.id
-- LEFT JOIN (
--      SELECT DISTINCT ON (i.datasource)
--      i.id, i.datasource, i.revision, r.noble
--      FROM imports i LEFT JOIN revisions r ON i.revision = r.digest
--      ORDER BY i.datasource, i.id DESC) i ON s.id = i.datasource
-- WHERE v.id = :id
-- ORDER BY v.id DESC

...
failed_imports AS (
         --TODO name->title
  SELECT spec->>'name' AS name, error_reason, job_execution.created, job_execution.modified
    FROM data_source, job_execution
   WHERE job_execution.data_source_id=data_source.id
     AND error_reason IS NOT NULL
),
pending_imports AS (
  SELECT spec->>'name' AS name, job_execution.created, job_execution.modified
    FROM data_source, job_execution
   WHERE job_execution.data_source_id=data_source.id
     AND finished_at IS NULL
)
SELECT NULL AS id, name, error_reason as "errorReason", 'FAILED' AS status, modified, created
  FROM failed_imports
 UNION
SELECT NULL, name, NULL, 'PENDING', modified, created
  FROM pending_imports
 UNION
SELECT id, title, NULL, 'SUCCESS', modified, created
  FROM dataset;

...
SELECT
q.id AS id, q.datasource AS datasource, q.dataset_name AS "name",
q.status, q.dataset_ts AS created,
GREATEST(q.import_ts, q.dataset_ts, q.meta_ts, q.transformation_ts) AS modified
FROM (
     SELECT DISTINCT ON (ds.id)
     ds.id AS id, j.datasource, m.dataset_name, j.status,
     j.ts AS import_ts, ds.ts AS dataset_ts, m.ts AS meta_ts,
     tf.ts AS transformation_ts
     FROM dataset ds
     LEFT JOIN (
...
q.status, q.dataset_ts AS created,
GREATEST(q.import_ts, q.dataset_ts, q.meta_ts, q.transformation_ts) AS modified,
q.digest, q.noble, q.transformations
FROM (
     SELECT DISTINCT ON (ds.id)
     ds.id AS id, j.datasource, m.dataset_name, j.status,
     j.ts AS import_ts, ds.ts AS dataset_ts, m.ts AS meta_ts,
     tf.ts AS transformation_ts,
     j.digest, j.noble, tf.id AS transformation_id, tf.fns AS transformations
     FROM dataset ds
     LEFT JOIN (
          SELECT DISTINCT ON (dsm.dataset)
          dsm.id, dsm.dataset_name, dsm.dataset, dsm.ts
     ...
     LEFT JOIN (
          SELECT DISTINCT ON (i.datasource)
          i.id, i.datasource, i.status, i.revision, i.ts, r.noble, r.digest
          FROM imports i LEFT JOIN revisions r ON i.revision = r.digest
          ORDER BY i.datasource, i.id DESC) j ON s.id = j.datasource
    WHERE ds.id = :id) q

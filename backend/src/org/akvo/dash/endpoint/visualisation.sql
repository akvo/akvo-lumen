...
-- :name delete-visualisation :! :n
-- :doc Insert a single visualisation data
INSERT INTO visualisation_data (id, visualisation, "name", spec, enabled)
VALUES (:id, :visualisation, :name, :spec::jsonb, :enabled);
...
SELECT v.id, vd.name, vd.spec, v.ts AS created, vd.ts AS modified
FROM visualisation v
LEFT JOIN (
     SELECT DISTINCT ON (v_data.visualisation)
     v_data.name, v_data.visualisation, v_data.spec, v_data.enabled, v_data.ts

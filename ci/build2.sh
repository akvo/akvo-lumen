...
docker run -v "$HOME/.m2:/home/akvo/.m2" -v "$(pwd)/backend:/app" "akvo/akvo-lumen-backend-dev:${backend_image_version}" /app/backend run-as-user.sh /bin/bash -c "cd /app/backend; clj -C:org.akvo.lumen.dev -R:org.akvo.lumen.dev  -A:org.akvo.lumen.test -e :functional"

...
while [ $(( $(date +%s) - 300 )) -lt "${starttime}" ]; do
   ...
   lumen_status=$(kubectl get pods -l "lumen-version=$TRAVIS_COMMIT,run=lumen-blue-green,color=${COLOR}" -o jsonpath='{range .items[*].status.containerStatuses[*]}{@.name}{" ready="}{@.ready}{"\n"}{end}')
...
# Another possibility could be to check that the service is pointing just to the new containers.
   old_lumen_status=$(kubectl get pods -l "lumen-version!=$TRAVIS_COMMIT,run=lumen-blue-green,color=${COLOR}" -o jsonpath='{range .items[*].status.containerStatuses[*]}{@.name}{" ready="}{@.ready}{"\n"}{end}')

...
read -p "Probably is either test or lumen (for production): " CLUSTER

if [ "${CLUSTER}" == "lumen" ]; then
 read -e -p "Are you sure you want to flip production? [yn] " CONFIRM
 if [ "${CONFIRM}" != "y" ]; then
    log "Nothing done"
    exit 1
 fi
...
NEW_LIVE_COLOR=$(${DIR}/helpers/dark-color.sh ${CURRENT_LIVE_COLOR})
if [ $? -ne 0 ]; then
